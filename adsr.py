def amp_envelop(note_dur, attack_time=0.08, decay_time=0.3, sustain_level=0.6, release_time=0.4, sr=44100):
    ...
    if note_dur < attack_time+release_time:
        env_release = np.logspace(np.log10(1),np.log10(0.001),int(release_time*sr))[:int((note_dur-attack_time)*sr)]
        envs.append(env_release)
    elif note_dur < attack_time+decay_time+release_time:
        env_decay = np.logspace(np.log10(1),np.log10(sustain_level),int(decay_time*sr))[:int((note_dur-attack_time-release_time)*sr)]
